...
export default function OrderQueryPage() {
  ...
  const handleShelves = async (type) => {
    setDialogType(type);
              ...
              <Stack spacing={2} sx={{ width: '100%' }}>
                {ticketAllList?.map((item, index) => (
                  <Grid
                    ...
                    <Card
                      sx={{ width: '100%', borderRadius: 3, p: 1, position: 'sticky', top: 20 }}>
                      <Box
                        sx={{
                          display: 'flex',
                      ...
                      <Box
                        ...
                        <Box
                          ...
                          <Chip
                            label={
                              item.vcStatus === 'PENDING'
                                ? '尚未領取票券'
                                : item.vcStatus === 'TRADING'
                                ? '票券販售中'
                                : item.vcStatus === 'ACTIVE'
                                ? '票券持有中'
                                : item.vcStatus === 'REVOKED'
                                ? '票券已撤銷'
                                : `其他狀態：${item.vcStatus}`
                            }
                            color={
