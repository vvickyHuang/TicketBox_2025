...
import { setTradingList } from '@/lib/features/globalSlice';
...
export default function LoginPage() {
  ...
  // const grandTotal = tradingList?.ticketList.reduce((sum, t) => sum + t.price, 0);

  const handleSubmit = async () => {
    console.log('Submitting payment with:', { card, expDate, cvc });
    console.log('Buy Info:', buyInfo);

    const tradeUuids = buyInfo.ticketList
      .map((item) => item.shelvesData?.tradeToken)
  ...
  useEffect(() => {
    if (tradingList) {
      sessionStorage.setItem('tradingList', JSON.stringify(tradingList));
    }
    const updatedTicketList = tradingList.ticketList.map((ticket) => ({
      ...ticket,
      email: '',
      user: '',
    }));
          ...
          >
            ...
            <Box sx={{ m: 2 }}>
              ...
              <Grid container sx={{ display: 'flex', flexDirection: 'column' }}>
                {/* {tradingList?.ticketList?.map((item, index) => (
                  <Grid
                    key={index}
                    sx={{
                      gap: 1,
                ...
                <Box sx={{ mb: 2 }}>
                  <TextField
                    ...
                    onChange={(e) => {
                      ...
                      const newTicketList = tradingList.ticketList.map((item) => ({
                        ...item,
                        email: value,
                      }));
